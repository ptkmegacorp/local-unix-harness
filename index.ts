...
function buildCfg(root: string, overrides: any = {}) {
  return {
    ...
    useLlmPresenter: Boolean(overrides.useLlmPresenter ?? ((process.env.HARNESS_USE_LLM_PRESENTER || "1") === "1")),
...
export default function register(api: any) {
  const pluginCfg = (api?.config?.plugins?.entries?.["local-unix-harness"]?.config) || {};
  const root = pluginCfg.root || "/home/bot/local-unix-harness";
  ...
  api.registerTool({
    ...
    async execute(_id: string, params: any) {
      try {
        const mod = await import("file:///home/bot/local-unix-harness/src/run.js");
  ...
  api.registerTool({
    ...
    async execute() {
      try {
        const mod = await import("file:///home/bot/local-unix-harness/src/presenter.js");
        const cfg = buildCfg(root, pluginCfg);
